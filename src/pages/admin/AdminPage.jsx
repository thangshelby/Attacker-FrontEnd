...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, PieChart as RechartsPieChart, Cell, BarChart, Bar, AreaChart, Area, Pie } from "recharts";

// Mock data
const monthlyData = [
  { month: 'T1', applications: 45, approved: 32, rejected: 8, pending: 5, totalAmount: 2.1 },
  { month: 'T2', applications: 52, approved: 38, rejected: 9, pending: 5, totalAmount: 2.8 },
  { month: 'T3', applications: 48, approved: 35, rejected: 7, pending: 6, totalAmount: 2.4 },
  { month: 'T4', applications: 65, approved: 45, rejected: 12, pending: 8, totalAmount: 3.2 },
  { month: 'T5', applications: 58, approved: 42, rejected: 10, pending: 6, totalAmount: 2.9 },
  { month: 'T6', applications: 72, approved: 55, rejected: 11, pending: 6, totalAmount: 3.8 },
  { month: 'T7', applications: 68, approved: 48, rejected: 15, pending: 5, totalAmount: 3.5 },
  { month: 'T8', applications: 75, approved: 58, rejected: 12, pending: 5, totalAmount: 4.1 },
];
...
const recentActivities = [
  { id: 1, type: 'approval', student: 'SV001', amount: '50,000,000', time: '5 phút trước', university: 'ĐH Kinh Tế - Luật' },
  { id: 2, type: 'rejection', student: 'SV002', amount: '30,000,000', time: '12 phút trước', university: 'ĐH Bách Khoa' },
  { id: 3, type: 'pending', student: 'SV003', amount: '40,000,000', time: '18 phút trước', university: 'ĐH Quốc Gia' },
  { id: 4, type: 'approval', student: 'SV004', amount: '25,000,000', time: '25 phút trước', university: 'ĐH Công Nghệ' },
  { id: 5, type: 'approval', student: 'SV005', amount: '35,000,000', time: '32 phút trước', university: 'ĐH Khoa học Tự nhiên' },
];
...
const StatCard = ({ title, value, change, changeType, icon: Icon, color, subtitle }) => {
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-white shadow-lg dark:bg-gray-800 transform hover:scale-105 transition-all duration-300">
      <div className="p-6">
...
const ActivityItem = ({ activity }) => {
  ...
  const getActivityText = (type) => {
    switch (type) {
      case 'approval': return 'được duyệt';
      case 'rejection': return 'bị từ chối';
    ...
    <div className="flex items-center space-x-4 p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-gray-900 dark:text-white">
          Khoản vay <span className="font-semibold">{activity.student}</span> {getActivityText(activity.type)}
        </p>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
              ...
              <div className="mb-6 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-600 dark:text-gray-400">Đơn vay</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="h-3 w-3 rounded-full bg-green-500"></div>
                    <span className="text-sm text-gray-600 dark:text-gray-400">Được duyệt</span>
                  </div>
                  <div className="flex items-center space-x-2">
                  ...
                  <div key={index} className="flex items-center justify-between p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">{uni.students} sinh viên</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold text-gray-900 dark:text-white">{uni.amount}B VNĐ</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">GPA: {uni.avgGPA}</p>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <div key={index} className="rounded-lg border p-4 dark:border-gray-600">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{agent.accuracy}%</span>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500 dark:text-gray-400">Quyết định:</span>
                        <span className="font-medium text-gray-900 dark:text-white">{agent.decisions}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500 dark:text-gray-400">Thời gian TB:</span>
                        <span className="font-medium text-gray-900 dark:text-white">{agent.avgTime}s</span>
                      </div>
          ...
          <div className="space-y-8">
            ...
            <div className="rounded-2xl bg-white p-6 shadow-lg dark:bg-gray-800">
              ...
              <button className="mt-4 w-full rounded-lg bg-gray-100 p-2 text-sm text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-600 transition-colors">
          ...
          <div className="rounded-2xl bg-white p-6 shadow-lg dark:bg-gray-800">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={monthlyData}>
                ...
                <Bar dataKey="totalAmount" fill="url(#colorGradient)" radius={[4, 4, 0, 0]} />
                <defs>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-lg dark:bg-gray-800">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlyData.map(month => ({
                ...month,
                successRate: ((month.approved / month.applications) * 100).toFixed(1)
              }))}>
                <CartesianGrid strokeDasharray="3 3" className="opacity-30" />
                <XAxis dataKey="month" />
                <YAxis domain={[0, 100]} />
